	...
	*/
	...
	class Usuario {
		...
		public function validar2(){
			$sql = "SELECT lu.*,lu.id,lu.nombre,lu.perfil FROM usuarios as lu WHERE lu.nombre='$this->email' AND lu.pass='$this->password' ";
			$datos = $this->con->consultaRetorno($sql);
			while($row = mysqli_fetch_array($datos,MYSQLI_ASSOC)) {
				$this->id        = $row['id'];
				$this->nombre    = $row['nombre'];
		...
		{
			$sql = "INSERT INTO usuarios (nombre, email, pass, perfil,estatus,fecha_registro)
